...
export class UserRepository
  ...
  implements UserRepositoryIFace
{
  ...
  constructor() {
    ...
    this.getDataSource = async () =>
      (await this.getDataSourceWithEntities(
        ...
        [preferenceEntity, projectEntitySchema],
        path.join(this.getTemplatesDirectory(), 'clear-aligner-user.sqlite'),
        this.getDataDirectory(),
        true
      ))!; // allow create should always be true for the user database
  }
  getProjects = async (): Promise<ProjectEntity[]> => {
    const repo: Repository<ProjectEntity> = (
  ...
  createOrUpdatePreferences = async (preferenceData: UserPreferenceDto) => {
    const preferenceRepository = (await this.getDataSource()).getRepository(
      'preference'
    );
