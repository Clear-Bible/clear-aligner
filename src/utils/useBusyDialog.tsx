...
const useBusyDialog = ({
  ...
  const refreshCounts = useCallback(() => {
    ...
    if (
      !_.isEqual(
        { projects: newProjectStatus, links: newLinkStatus },
        databaseStatus
      )
    ) {
      setDatabaseStatus({
    ...
    projectState?.projectTable?.getProjects(false).then((newProjects) => {
  ...
  const busyCountAtLeastTwo = useMemo<boolean>(
    () =>
      [busyCount.projects, busyCount.links].some((count): boolean => count > 1),
  ...
  const spinnerParams = useMemo<{
    ...
    const busyInfo = [databaseStatus?.links, databaseStatus?.projects]
      .filter(Boolean)
    ...
    if (busyInfo?.isBusy) {
      ...
      if (progressMax > 0 && progressMax >= progressCtr) {
    ...
    customStatus,
    busyCountAtLeastTwo,
    databaseStatus?.links,
    databaseStatus?.projects,
  ]);
  ...
  const isOpen = useMemo(
    () => isForceShowBusyDialog || (!!spinnerParams.isBusy && !cancel),
    [isForceShowBusyDialog, spinnerParams.isBusy, cancel]
  );
    ...
    busyDialog: (
      ...
      >
        ...
        <DialogContent>
          ...
          <DialogContentText>{spinnerParams.text}</DialogContentText>
        </DialogContent>
      </Dialog>
    ),
  };
