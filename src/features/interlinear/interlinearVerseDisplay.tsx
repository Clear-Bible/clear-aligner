...
import {
  createInterlinearMap,
  InterlinearMap,
} from '../../helpers/createInterlinearMap';
...
const determineDefaultTargetView = (
  ...
  return (
    <Grid
      item
      key={`interlinear/${sourceCorpus?.id}/${sourceTokens
...
const determineTargetView = (
  ...
  if (
    !interlinearMap.containers.isComplete() ||
  ...
  const targetTokens = sourceTokens
    .map((sourceToken) => {
      ...
      const sortedTokens = [...linkWords.words].sort((leftWord, rightWord) =>
      ...
      const outputText = compressedTokens
        .map((compressedToken) => compressedToken.text)
        .join(' ');
      return {
        ...compressedTokens[0],
        text: outputText,
  ...
  const targetCorpus =
    interlinearMap.containers.targets?.corpusAtReferenceString(
      targetTokens[0].id
    );

  // no links or corpus = empty target cell
  if (targetLinkMap.size < 1 || !targetCorpus) {
    return determineDefaultTargetView(sourceCorpus, sourceTokens);
  }
  ...
  return (
    <Grid
      item
      key={`interlinear/${targetCorpus?.id}/${targetTokens
 ...
 */
const determineVerseView = (interlinearMap: InterlinearMap) => {
  ...
  const sourceCorpus =
    interlinearMap.containers.sources?.corpusAtReferenceString(verseRef);
  if (!sourceCorpus) {
    return [];
  }
  ...
  const verseTokens: Word[][] = groupPartsIntoWords(
    interlinearMap.sourceVerse.words
  );
  return (verseTokens || []).map(
    (sourceTokens: Word[]): ReactElement => (
...
export const InterlinearVerseDisplay = ({
  ...
  const [interlinearMap, setInterlinearMap] = useState<
    InterlinearMap | undefined
  >();
  ...
  useEffect(() => {
    ...
    createInterlinearMap(projectState.linksTable, verse, containers).then(
      setInterlinearMap
    );
  ...
  useEffect(() => {
    ...
    setVerseElements(determineVerseView(interlinearMap));
  ...
  return (
    ...
    >
      {verseElements?.map((verseElement, verseIndex) => (
        <Grid item key={`interlinear/${verseIndex}`}>
          {verseElement}
        </Grid>
      ))}
    </Grid>
