...
export const useAlignedWordsFromPivotWord = (pivotWord?: PivotWord, sort?: GridSortItem | null, useLemma = false): AlignedWord[] | undefined => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setAlignedWords(alignedWords.map((aw): AlignedWord => ({
        id: `${aw.t}:${aw.st}-${aw.tt}`,
        sourceWordTexts: {
