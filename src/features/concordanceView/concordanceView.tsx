...
import { CropFree, GpsFixed, InsertLink } from '@mui/icons-material';
...
const SaveChangesConfirmationViaRouter = ({
  blocker,
}: SaveChangesConfirmationViaRouterProps) => {
  if (!blocker) {
    return;
  }
  ...
  return (
    <>
      <Stack direction="row" justifyContent="start" alignItems="center">
        ...
        <Dialog maxWidth="lg" open={isDialogOpen} disableEscapeKeyDown={true}>
          <Box padding={5}>
            <DialogContent sx={{ height: '100%', width: '100%' }}>
              Save {linksPendingUpdate.size}{' '}
              {linksPendingUpdate.size === 1 ? `change` : `changes`}?
            </DialogContent>
            <DialogActions>
...
export const ConcordanceView = () => {
  ...
  const [linksPendingUpdate, setLinksPendingUpdate] = useState<
    Map<string, RepositoryLink>
  >(new Map());
  const [updatedSelectedRows, setUpdatedSelectedRows] = useState<
    RepositoryLink[]
  >([]);
  const { pivotWords } = usePivotWords(
    wordSource,
    wordFilter,
    pivotWordSortData,
    lemmaToggled && allowLemmaToggle
  );
  ...
  const handleUpdateSelectedAlignedWord = useCallback(
    async (alignedWord: AlignedWord | null) => {
      if (linksPendingUpdate.size > 0 && alignedWord !== null) {
        // open modal asking if users want to continue or not
        const status = await getSaveChangesConfirmation(
          'Changes will be lost if you navigate away. Continue?'
        );
  ...
  const handleUpdateSelectedPivotWord = useCallback(
  ...
  useEffect(() => {
    ...
    } else if (searchParams.has('alignmentLink') && selectedAlignedWord) {
  ...
  React.useEffect(() => {
    if (preferences?.currentProject) {
      dbApi
        .getDataSourceLemmaCount(preferences.currentProject)
        .then((lc) => {
                ...
                <Grid item xs={pivotWordTableColumns}>
                  ...
                  >
                    <Box>
                      <SingleSelectButtonGroup
                        ...
                        onSelect={(value) => {
                          ...
                          if (
                            alignmentSide === AlignmentSide.TARGET &&
                            lemmaToggled
                          ) {
                    ...
                    <FormControl sx={{ marginLeft: '6px', display: 'inline' }}>
                      <InputLabel id={'pivot-word-filter'}>
                        Pivot Word Filter
                      </InputLabel>
                      <Select
                      ...
                      >
                        <MenuItem value={'aligned' as PivotWordFilter}>
                          <Box display={'flex'}>
                            <ListItemIcon
                              sx={{ minWidth: '36px', alignItems: 'center' }}
                            >
                ...
                <Grid item xs={alignmentLinkTableColumns}>
                  ...
                  >
                    <AlignmentTableControlPanel
                      ...
                      alignmentTableControlPanelLinkState={
                        alignmentTableControlPanelLinkState || null
                      }
                      setAlignmentTableControlPanelLinkState={
                        setAlignmentTableControlPanelLinkState
                      }
          ...
          <Grid item xs={pivotWordTableColumns}>
            ...
            >
              <PivotWordTable
                ...
                onChooseWord={(word) => handleUpdateSelectedPivotWord(word)}
