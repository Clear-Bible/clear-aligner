...
export const useSyncAlignments = (): SyncState => {
  ...
  const [syncedAlignments, setSyncedAlignments] = useState<
  ...
  const sendJournal = useCallback(
    async (signal: AbortSignal, projectId?: string) => {
      try {
        const journalEntriesToUpload = await dbApi.getAllJournalEntries(
          projectId!
        );
        ...
        await dbApi.deleteByIds({
          ...
          itemIdOrIds: journalEntriesToUpload.map(
  ...
  const syncLinks = useCallback(
    async (alignmentProjectId?: string, controller?: AbortController) => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <Typography variant="subtitle1">Syncing Alignments...</Typography>
          <Button
            variant="text"
