 ...
 */
export const mockSetPreferences = (
  initialPrefs?: Partial<UserPreference>,
  onChange?: (p: UserPreference) => void
): React.Dispatch<React.SetStateAction<UserPreference | undefined>> => {
  const initialValue = mockPreferences(initialPrefs);
  return ((
    s: UserPreference | ((p: UserPreference | undefined) => UserPreference)
  ) => {
